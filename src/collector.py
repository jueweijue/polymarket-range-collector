...
def resolve_markets(start_ts: int, end_ts: int) -> List[MarketInfo]:
    """Resolve BTC Up/Down 5m markets by querying Gamma API.

    Pages Gamma /markets by day-range and filters locally for btc-updown-5m-.
    """
...
def collect_historical(project_dir: Path, config: dict):
    ...
    for idx, market in enumerate(markets, start=1):
        ...
        if not trades:
            log(f"[history] [{idx}/{total}] {market.slug}: trades=0, skip cache write")
